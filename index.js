          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            break
                ...
                addKoinUser('558499278703@s.whatsapp.net', fee)
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Marque a foto com ${prefix}𝗼𝗰𝗿')
					}
					await limitAdd(sender)
					break
				case 'stiker': 
				...
				case 's':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`marque uma foto/gif com ${prefix}`)
					}
					break
				case 'tts':
				if (!isRegistered) return reply(ind.noregis())
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					reply(`Prefixo alertado para : ${prefix}`)
					break 
				case 'tiktokstalk':
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar esse corno, deve ter botado privado')
					}
					break
					case 'grup':
					case 'group':
           ...
           case 'setname':
                ...
                client.sendMessage(from, 'Sucesso ao alterar a descrição', text, {quoted: mek})
					break
           case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `*perdeu adm otario*🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`𝘆𝗮𝗵𝗵 @${mentioned[0].split('@')[0]} que? seu adm ja foi retirado🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `NOVO ADM (RESPEITA ESSE PUTO (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabens🥳 @${mentioned[0].split('@')[0]} *AGORA E UM ADIMIN NO GRUPO* (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Bani mesmo esse corno 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Bani mesmo esse corno @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `𝗟𝗶𝘀𝘁a de cornos do grupo *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨CESSO ❭ NSFW ATIVO')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('❬ 𝗦UCESSO ❭ NSFW ATIVO')
					} else {
						reply(ind.satukos())
					}
					break
                ...
                if (args[0] === 'enable') {
                 ...
                 case 'block':
					...
					client.sendMessage(from, `COMANDO ACEITO, BLOQUEI MESMO wa.me${body.slice(8)}@c.us`, text)
					break
				case 'unblock':
					client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, `Comando aceito, desblqueado com sucesso wa.me/${body.slice(10)}`, text)
					break
				case 'welcome':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨CESSO ❭ ATIVADO O RECURSO DE BEM VINDAS/SAIDA')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUCESSO ❭ ATIVADO O RECURSO DE BEM VINDAS/SAIDA')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*❬ 𝗦𝗨CESSO❭ 𝗠𝗲𝗻𝗴𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 EVENT 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ 𝗦𝗨CESSO ❭ 𝗠𝗲𝗻𝗼𝗻𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 EVENT 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️*')
					} else {
						reply(ind.satukos())
					}
					break
				case 'clone':
					...
					if (args.length < 1) return reply(' *MARQUE QUEM QUER CLONAR!!!* ')
					...
					try {
						...
						mentions(`Foto Atualizada, Roubei a foto desse corno @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply(ind.stikga())
					}
					await limitAdd(sender)
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`Desculpe *${pushname}*, Comando *${prefix}${command}* Comando não registrado no *${prefix}menu*!`)
